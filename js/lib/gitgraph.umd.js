...
(function (global, factory) {
	...
	class Commit {
	    constructor(options) {
			...
			this.commitMessageBgColor=options.commitMessageBgColor;
	    ...
	    cloneCommit() {
			...
			commitMessageBgColor:this.commitMessageBgColor
	        });
	...
	class GitgraphCore {
	    constructor(options = {}) {
	        ...
	        this.author = options.author || "<saxo-guy@epic.com>";
	        this.commitMessage =
	            options.commitMessage || "He  George Michael! Boooo!";
	...
	function createPath(options) {
		...
		if(options.onClick){
			path.addEventListener("click",options.onClick);
		}
		if(options.onMouseOver){
			path.addEventListener("mouseover",options.onMouseOver);
		}
		if(options.onMouseOut){
			path.addEventListener("mouseout",options.onMouseOut);
		}
	    return path;
	}
	...
	function createTag(tag) {
	    ...
	    var observer = new MutationObserver(function () {
	        ...
	        var boxHeight = height + 2 * PADDING_Y$1-40;
	        var pathD = [
	            "M 0,0",
	...
	function createGitgraph(graphContainer, options) {
	        ...
	        var paths = Array.from(branchesPaths).map(function (_a) {
	            ...
	            return createPath({
					...
					onMouseOver:function(){
						branch.onMouseOver(branch);

					},
					onMouseOut:function(){
						branch.onMouseOut(branch);
	    ...
	    function renderCommits(commits) {
			let branches__=commits[0].branches;

	        return createG({ children: commits.map(renderCommit) });
	        function renderCommit(commit) {
				let indexof=branches__.indexOf(commit.branchName);
	            ...
	            return createG({
	                ...
	                children: [
	                    ...
	                    createG({
	                        ...
	                        // translate: { x: -x-((branches__.length-indexof-1)*25), y: 0 },
	                        children: [
	                            renderMessage(commit)
	                        ].concat(renderBranchLabels(commit,x,0), renderTags(commit)),
	                    }),
	                ]),
	            });
	        }
	            ...
	            return commit.parents.map(function (parentHash) {
	                ...
	                var origin = gitgraph.reverseArrow
	                    ? {
	                        ...
	                        y: commitRadius + (parent.y - commit.y)-40,
	                    }
	    ...
	    function renderMessage(commit) {
			...
			var rect = createRect({
				...
				height: 0,
				stroke: 'black',
				fill: commit.commitMessageBgColor||"",
			});
	        var text = createText({
			...
			if(!commit.commitMessageBgColor){
				message = createG({
					// hash:commit.hash,
					onClick:()=>{
	    ...
	    function renderBranchLabels(commit,x,y) {
	        ...
	        return branches.map(function (branch) {
	            ...
	            if (!gitgraph.branchLabelOnEveryCommit) {
					...
					if(commit.branchName===branch.name && branch.isCurrent){
